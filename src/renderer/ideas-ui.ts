export const purposeLabels: Record<IdeaPurpose, string> = {
  both: "Portfólio e uso prático",
  practical: "Uso prático em projetos",
  portfolio: "Portfólio e aprendizado",
};
export const purposes: IdeaPurpose[] = ["both", "practical", "portfolio"];
export const coverageLabels: Record<string, string> = {
  main_text: "texto completo",
  user_text: "texto colado por você",
  partial: "captura parcial",
  metadata_only: "só título e descrição",
};
...
export const statusLabels: Record<string, string> = {
  recommendation: "Ideia",
  insufficient_context: "Contexto insuficiente",
  no_application: "Nenhuma aplicação convincente",
};
export const outcomeLabels: Record<IdeaOutcome, string> = {
  recommendation: "Ideia pronta",
  insufficient_context: "Contexto insuficiente para uma ideia",
  no_application: "Nenhuma aplicação convincente",
...
export const icons = {
  ...
  smallClose:
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18"></path></svg>',
  ...
  refresh:
    '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 12a9 9 0 1 1-2.6-6.4M21 4v5h-5"></path></svg>',
  arrow:
...
export function errorText(error: unknown) {
  return String((error as Error)?.message ?? error).replace(
    /^Error: (Error invoking remote method '[^']+': )?(Error: |ConsentError: |ContractError: )?/,
    "",
  );
}
...
export function toast(
  ...
  window.dispatchEvent(
    new CustomEvent("loounp:toast", { detail: { message, action } }),
  );
}
