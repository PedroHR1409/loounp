...
const topicMetadata = new WeakMap<
  HTMLElement,
...
const svg = {
  ...
  notUseful:
    '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M17 13V4h3v9zM17 13l-4 8a2 2 0 0 1-2-2v-4H5a2 2 0 0 1-2-2.3l1.2-7A2 2 0 0 1 6.2 4H17"></path></svg>',
  external:
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7 17L17 7M9 7h8v8"></path></svg>',
  arrow:
    '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 12h14M13 6l6 6-6 6"></path></svg>',
  ...
  check:
    '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6L9 17l-5-5"></path></svg>',
};

function isV2Profile(
  profile: AppState["profile"] | InterestProfileV2 | null | undefined,
): profile is InterestProfileV2 {
  return Boolean(
    profile && "schemaVersion" in profile && profile.schemaVersion === 2,
  );
}
...
function notify(
  ...
  toastTimer = window.setTimeout(
    () => toast.classList.remove("show"),
    action || undoContentId ? 8000 : 3000,
  );
}
...
function notifyIdea(title: string, articleTitle: string, operationId: string) {
  ...
  later.addEventListener("click", () => toast.classList.remove("show"), {
    once: true,
  });
  ...
  open.addEventListener(
    ...
    () => {
      toast.classList.remove("show");
      void showView("ideas", operationId);
    },
    { once: true },
  );
...
function categoryLabel(value: string | null) {
  const labels: Record<string, string> = {
    news: "Notícia",
    "deep-dive": "Aprofundamento técnico",
    "technical-deep-dive": "Aprofundamento técnico",
    tutorial: "Tutorial",
    opinion: "Opinião",
    announcement: "Anúncio",
    "product-announcement": "Anúncio de produto",
    other: "Outro formato",
    uncertain: "Classificação incerta",
    promotional: "Sinais promocionais",
    repetitive: "Sinais de repetição",
...
function visibleItems() {
  ...
  if (activeFilter === "ideas")
    return state.items.filter((item) => ideasByContent.has(item.id));
  return state.items;
}
...
function renderCounts() {
  document.querySelector("#count-all")!.textContent = String(
    state.items.length,
  );
  document.querySelector("#count-saved")!.textContent = String(
    state.items.filter((item) => item.saved).length,
  );
  document.querySelector("#count-with-ideas")!.textContent = String(
    state.items.filter((item) => ideasByContent.has(item.id)).length,
  );
  document.querySelector("#count-feed")!.textContent = String(
    state.items.length,
  );
  const totalIdeas = [...ideasByContent.values()].reduce(
    (sum, count) => sum + count,
    0,
  );
  document.querySelector("#count-ideas")!.textContent = totalIdeas
    ? String(totalIdeas)
    : "";
...
function renderFeed() {
  ...
  if (!items.length) {
    const empty =
      activeFilter === "saved"
        ? [
            "Nada salvo ainda.",
            "Salve artigos para montar sua lista de leitura.",
          ]
        : activeFilter === "ideas"
          ? [
              "Nenhum artigo virou ideia ainda.",
              "Abra um artigo e clique em Gerar ideia.",
            ]
          : [
              "Vamos encontrar boas leituras.",
              "Ajuste seus interesses e busque publicações recentes no Dev.to e no Medium.",
            ];
  ...
  feed.innerHTML = shown
    .map((item) => {
      ...
      const chips = [
        ...
        item.isExploratory
          ? '<span class="chip">Descoberta adjacente</span>'
          : "",
        ideas
          ? `<span class="chip chip-idea">${svg.idea}${ideas} ideia${ideas > 1 ? "s" : ""}</span>`
          : "",
      ].join("");
      const progress =
        generating?.contentId === item.id
          ? `<div class="row-progress"><span class="spin" aria-hidden="true"></span><span>Gerando ideia…</span><button type="button" class="text-btn" data-action="cancel-generation">Cancelar</button></div>`
          : "";
      const meta = [
  ...
  if (items.length > shown.length)
    feed.insertAdjacentHTML(
      "beforeend",
      `<div class="feed-more"><button type="button" class="btn" data-action="show-more">Ver mais ${Math.min(20, items.length - shown.length)} de ${items.length - shown.length}</button></div>`,
    );
}
...
function renderReading() {
  ...
  if (!item) {
    readingPane.innerHTML =
      '<div class="reading-empty"><h2>Nada selecionado</h2><p>Escolha um artigo na lista para ler o resumo e transformá-lo em ideia de projeto.</p></div>';
    return;
  }
  const meta = [
    escapeHtml(item.author),
    relativeDate(item.publishedAt),
    item.readingMinutes ? `${item.readingMinutes} min de leitura` : "",
  ]
    .filter(Boolean)
    .join(" · ");
  const scoreEvidence = [
    item.personalUtility == null
      ? ""
      : `Utilidade pessoal ${Math.round(item.personalUtility * 100)}%`,
    item.topicFit == null
      ? ""
      : `Aderência temática ${Math.round(item.topicFit * 100)}%`,
    item.technicalSubstance == null
      ? ""
      : `Substância técnica ${Math.round(item.technicalSubstance * 100)}%`,
  ].filter(Boolean);
  const reasons = [...item.reasons.slice(0, 4), ...scoreEvidence]
    .map((reason) => `<span class="chip">${escapeHtml(reason)}</span>`)
    .join("");
  const analysis =
    item.summary || item.category || item.hypeEvidence.length
      ...
      : "";
  const jev = item.jevAssessment
    ? `<div class="analysis"><strong>Jev · ${item.jevAssessment.status === "partial" ? "avaliação parcial" : "avaliação válida"}</strong><span>Utilidade ${escapeHtml(item.jevAssessment.utility?.level ?? "neutra")} · profundidade ${escapeHtml(item.jevAssessment.technicalDepth?.level ?? "neutra")}</span></div>`
    : "";
  const ideas = ideasByContent.get(item.id) ?? 0;
...
async function loadIdeaSummary() {
  const history = await window.projectIdeas
    .history()
    .catch(() => [] as IdeaHistoryItem[]);
  ideasByContent = new Map();
  for (const item of history)
    if (item.contentId)
      ideasByContent.set(
        item.contentId,
        (ideasByContent.get(item.contentId) ?? 0) + 1,
      );
}

async function loadContextSummary() {
  const [status, memory] = await Promise.all([
    window.projectIdeas.contextStatus().catch(() => null),
    window.projectIdeas.listMemory().catch(() => null),
  ]);
    ...
    ? {
        ...
        memories: memory
          ? memory.memories.filter(
              (entry) => entry.confirmed && !entry.ignoredInIdeas,
            ).length
          : 0,
...
setIdeaPanelHandlers({
  ...
  onGenerationStarted: (operationId, contentId) => {
    generating = { operationId, contentId };
    render();
  },
});
setIdeasViewHandlers({
  reviewOperation: (operationId) => {
    void showView("feed").then(() => openIdeaPanelOperation(operationId));
  },
  feedTitle: (contentId) =>
...
window.projectIdeas.onEvent((event) => {
  ...
  if (event.type === "state") {
    if (event.state === "generating" || event.state === "validating")
      generating = {
  ...
  if (event.contextMode === "article_only") {
    if (event.outcome !== "canceled")
      notify(
        "Versão sem contexto pronta. Compare as duas em Ideias.",
        undefined,
        event.comparisonOf
          ? {
              label: "Comparar",
              run: () => void showView("ideas", event.comparisonOf!),
            }
          : undefined,
      );
    return;
  }
  if (event.outcome === "recommendation") {
    notifyIdea(event.title, event.articleTitle, event.operationId);
    return;
  }
  if (
    event.outcome === "insufficient_context" ||
    event.outcome === "no_application" ||
    event.outcome === "failed"
  ) {
    notify(
      `${outcomeLabels[event.outcome]} · ${event.articleTitle}${event.outcome === "failed" && event.error ? `: ${event.error}` : ""}`,
      undefined,
      { label: "Ver em Ideias", run: () => void showView("ideas") },
    );
  }
});
...
void window.projectIdeas
  ...
  .then((current) => {
    if (!current) return;
    if (
      current.operation.state === "generating" ||
      current.operation.state === "validating"
    ) {
      generating = {
        operationId: current.operation.id,
        contentId: current.operation.contentId,
      };
      if (state) render();
    }
    ...
    if (current.operation.state === "interrupted" && !seen) {
      notify(
        `A geração de "${current.operation.article?.title ?? "um artigo"}" foi interrompida quando o app fechou; nada foi reenviado.`,
      );
      try {
...
function addTopic(
  name = "",
  importance = 3,
  metadata?: InterestProfileV2["interestGroups"][number],
) {
...
function showSettings() {
  ...
  const profile = isV2Profile(state.draftProfile)
    ? state.draftProfile
    : isV2Profile(state.profile)
      ? state.profile
      : null;
  ...
  if (profile) {
    (document.querySelector("#intent-text") as HTMLTextAreaElement).value =
      profile.intentText;
    (
      document.querySelector("#positive-examples") as HTMLTextAreaElement
    ).value = profile.examples
      .filter((example) => example.polarity === "positive")
      .map((example) =>
        [
          example.url,
          [example.title, example.excerpt].filter(Boolean).join(" — "),
        ]
          .filter(Boolean)
          .join(" — "),
      )
      .join("\n");
    (
      document.querySelector("#negative-examples") as HTMLTextAreaElement
    ).value = profile.examples
  ...
  (document.querySelector("#medium-feeds") as HTMLTextAreaElement).value = (
    profile?.mediumFeeds ??
    (isV2Profile(state.profile) ? [] : state.profile.mediumFeeds)
  ).join("\n");
  const slider = document.querySelector<HTMLInputElement>("#recency")!;
  slider.value = String(
    Math.round(
      (profile?.recencyPreference ??
        (isV2Profile(state.profile) ? 0.55 : state.profile.recencyPreference)) *
        100,
    ),
  );
  document.querySelector("#recency-label")!.textContent =
    `${slider.value}% novidade`;
  document
    .querySelector("#api-key")!
    .setAttribute(
      "placeholder",
      state.hasApiKey ? "Chave salva com segurança" : "Chave de API (opcional)",
    );
  document
    .querySelector("#jev-key")!
    .setAttribute(
      "placeholder",
      state.hasJevKey
        ? "Chave TypeSafe salva com segurança"
        : "Chave API TypeSafe (acesso antecipado)",
    );
...
async function load() {
  ...
  document.querySelector("#ranking-status")!.textContent =
    state.rankingSource === "jev"
      ? "ranking Jev"
      : state.rankingSource === "fallback"
        ? "ranking base (Jev indisponível)"
        : "ranking base";
  document.querySelector("#last-refresh")!.textContent = state.lastRefresh
    ? `Atualizado ${relativeDate(state.lastRefresh)}, ${new Date(state.lastRefresh).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}`
    : "Nenhuma busca ainda";
  if (currentView === "feed") render();
  else renderCounts();
...
document.querySelector("#refresh")!.addEventListener("click", async () => {
  ...
  try {
    ...
    await load();
    notify(
      `${result.added} novos artigos encontrados${result.errors.length ? ` · ${result.errors.length} fonte(s) falharam` : ""}`,
    );
    if (result.errors.length)
      console.warn("Falhas na ingestão:", result.errors);
  } catch (error) {
    notify(String(error));
  } finally {
    button.disabled = false;
    label.textContent = "Buscar novos";
  }
});
...
document
  .querySelector("#brand")!
  .addEventListener("click", () => void showView("feed"));
navButtons.feed.addEventListener("click", () => void showView("feed"));
navButtons.ideas.addEventListener(
  "click",
  () => void (currentView === "ideas" ? refreshIdeasView() : showView("ideas")),
);
navButtons.context.addEventListener(
  "click",
  () =>
...
document.querySelector("#recency")!.addEventListener("input", (event) => {
  document.querySelector("#recency-label")!.textContent =
    `${(event.target as HTMLInputElement).value}% novidade`;
});
document
  ...
  .addEventListener("submit", async (event) => {
    if (
      ...
      ((event as SubmitEvent).submitter as HTMLButtonElement).value !== "save"
    )
      return;
    ...
    const mediumFeeds = (
      document.querySelector("#medium-feeds") as HTMLTextAreaElement
    ).value
      .split(/\r?\n/)
      .map((feed) => feed.trim())
      .filter(Boolean);
    try {
      const existing = isV2Profile(state.draftProfile)
        ? state.draftProfile
        : isV2Profile(state.profile)
          ? state.profile
          : null;
      {
        const previousGroups = new Map(
          (existing?.interestGroups ?? []).map((group) => [
          ...
          return [
            {
              ...(previous ?? {
                summary: input.name,
                objectives: [input.name],
                subtopics: [input.name],
        ...
        const parseExamples = (
          ...
          (
            ...
            .map((raw) => {
              ...
              const detail = (
                url ? text.replace(url, "").trim().replace(/^—\s*/, "") : text
              ).slice(0, 1500);
              ...
              return {
                ...
                excerpt: excerpt.join(" — ").trim(),
              };
            })
            .filter((example) => example.title || example.url)
            .slice(0, 10);
        const examples = [
          ...parseExamples("#positive-examples", "positive"),
          ...parseExamples("#negative-examples", "negative"),
        ];
        const base = existing ?? {
        ...
        await window.contentApp.confirmProfile({
          ...
          intentText:
            document
              .querySelector<HTMLTextAreaElement>("#intent-text")!
              .value.trim() ||
            base.intentText ||
            "My selected interests",
          interestGroups,
          examples: examples.length ? examples : base.examples,
          mediumFeeds,
          recencyPreference:
            Number(
              (document.querySelector("#recency") as HTMLInputElement).value,
            ) / 100,
        });
      }
      dialog.close();
      await load();
      notify("Seus interesses foram atualizados.");
    } catch (error) {
      notify(String(error));
    }
  });
...
document
  .querySelector<HTMLButtonElement>("#propose-interests")!
  .addEventListener("click", async (event) => {
    const button = event.currentTarget as HTMLButtonElement;
    const status = document.querySelector<HTMLElement>("#proposal-status")!;
    const splitExamples = (selector: string) =>
      (
        document
          .querySelector<HTMLTextAreaElement>(selector)!
          .value.match(/[^\r\n]+/g) ?? []
    ...
    try {
      ...
      await load();
      showSettings();
      document.querySelector<HTMLElement>("#proposal-status")!.textContent =
        "Proposta pronta. Revise os grupos e exemplos antes de salvar.";
    } catch (error) {
...
document.querySelector("#save-key")!.addEventListener("click", async () => {
  ...
  try {
    await window.contentApp.setApiKey(input.value);
    input.value = "";
    await load();
    showSettings();
    notify("Chave salva com criptografia do sistema.");
  } catch (error) {
    notify(String(error));
  }
});
document.querySelector("#remove-key")!.addEventListener("click", async () => {
  await window.contentApp.clearApiKey();
  await load();
  showSettings();
...
document.querySelector("#save-jev-key")!.addEventListener("click", async () => {
  ...
  try {
    await window.contentApp.setJevKey(input.value);
    input.value = "";
    await load();
    showSettings();
    notify("Chave TypeSafe salva com criptografia do sistema.");
  } catch (error) {
    notify(String(error));
  }
});
document
  .querySelector("#remove-jev-key")!
  .addEventListener("click", async () => {
    await window.contentApp.clearJevKey();
    await load();
    showSettings();
    notify("Chave Jev removida.");
  });

let mediumArchiveReady = false;
document
  .querySelector<HTMLButtonElement>("#preview-medium-archive")!
  .addEventListener("click", async (event) => {
    const button = event.currentTarget as HTMLButtonElement;
    const preview = document.querySelector<HTMLElement>(
      "#medium-archive-preview",
    )!;
    const confirm = document.querySelector<HTMLButtonElement>(
      "#confirm-medium-archive",
    )!;
    button.disabled = true;
...
document
  .querySelector<HTMLButtonElement>("#confirm-medium-archive")!
  .addEventListener("click", async (event) => {
    const button = event.currentTarget as HTMLButtonElement;
    ...
    try {
      ...
      mediumArchiveReady = false;
      document.querySelector<HTMLElement>("#medium-archive-preview")!.hidden =
        true;
      await load();
      showSettings();
      notify(
        `${result.imported} artigos importados; ${result.learnedFrom} sinais de preferência adicionados. Faça uma nova busca para usar os exemplos na descoberta.`,
      );
    } catch (error) {
      notify(String(error));
    } finally {
...
feed.addEventListener("click", async (event) => {
  ...
  if (action === "cancel-generation" && generating) {
    try {
      await window.projectIdeas.cancel(generating.operationId);
    } catch (error) {
      notify(String(error));
    }
    return;
...
readingPane.addEventListener("click", async (event) => {
  ...
  if (action === "idea") {
    void openIdeaPanel({ id: item.id, title: item.title, url: item.url });
    return;
  }
  if (action === "view-ideas") {
    ...
    void showView(
      "ideas",
      history.find((entry) => entry.contentId === item.id)?.operationId,
    );
    return;
  }
  ...
  try {
    if (action === "open") {
      await window.contentApp.recordFeedback(item.id, "open");
      await window.contentApp.openLink(item.url);
    }
    if (action === "save")
      await window.contentApp.recordFeedback(
        item.id,
        item.saved ? "unsave" : "save",
      );
    if (action === "useful" || action === "not_useful")
      await window.contentApp.recordFeedback(item.id, "rate", action);
    if (action === "hide")
      await window.contentApp.recordFeedback(item.id, "hide");
    if (action === "analyze") {
      target.textContent = "Resumindo…";
      await window.contentApp.analyze(item.id);
    }
    await load();
