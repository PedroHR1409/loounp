...
): ArticleReaderDeps & { calls: string[] } {
  const calls: string[] = [];
  return {
    calls,
    resolve: async (hostname) =>
      (dns[hostname] ?? ["93.184.216.34"]).map((address) => ({
        address,
        family: address.includes(":") ? 6 : 4,
      })),
    transport: async (url, address) => {
      calls.push(`${url.toString()}@${address.address}`);
      return routes[url.toString()] ?? response(404);
    },
    now: () => now,
  };
}
...
describe("public document fetch", () => {
  ...
  it("rejects hosts that resolve to any private address (DNS rebinding) and credentialed URLs", async () => {
    await expect(
      fetchPublicDocument(
        "https://evil.test/",
        deps({}, { "evil.test": ["93.184.216.34", "127.0.0.1"] }),
      ),
    ).rejects.toThrow("privado");
    await expect(
      fetchPublicDocument("https://user:pw@example.com/", deps({})),
    ).rejects.toThrow("credenciais");
    await expect(
      fetchPublicDocument("http://localhost/", deps({})),
    ).rejects.toThrow(ArticleFetchError);
    await expect(fetchPublicDocument("file:///C:/x", deps({}))).rejects.toThrow(
      "HTTP",
    );
    await expect(
      fetchPublicDocument("https://example.com:8443/", deps({})),
    ).rejects.toThrow("Porta");
  });

  it("revalidates each redirect and stops after three", async () => {
    const toPrivate = deps({
      "https://example.com/a": response(302, "", {
        location: "http://169.254.169.254/latest",
      }),
    });
    await expect(
      fetchPublicDocument("https://example.com/a", toPrivate),
    ).rejects.toThrow("privado");
    const loop = deps({
      "https://example.com/1": response(301, "", { location: "/2" }),
      "https://example.com/2": response(301, "", { location: "/3" }),
      "https://example.com/3": response(301, "", { location: "/4" }),
      "https://example.com/4": response(301, "", { location: "/5" }),
    });
    await expect(
      fetchPublicDocument("https://example.com/1", loop),
    ).rejects.toThrow("Redirecionamentos");
  });
  ...
  it("enforces the response size limit and content type", async () => {
    await expect(
      fetchPublicDocument(
        ...
        deps({
          "https://example.com/big": response(200, "x".repeat(6 * 1024 * 1024)),
        }),
      ),
    ...
    await expect(
      fetchPublicDocument(
        ...
        deps({
          "https://example.com/pdf": response(200, "%PDF", {
...
describe("coverage labels (AT-03)", () => {
  it("labels paywalled pages as partial and empty pages as metadata only", () => {
    const paywalled = snapshotFromHtml(
      longArticle.replace("<article>", "<article><p>Member-only story</p>"),
      { kind: "url", url: "https://medium.com/x" },
      now,
    );
    expect(paywalled.coverage).toBe("partial");
    const empty = snapshotFromHtml(
      '<html><head><title>Só título</title><meta name="description" content="Descrição pública"></head><body></body></html>',
      { kind: "url", url: "https://x.test" },
      now,
    );
    expect(empty.coverage).toBe("metadata_only");
    expect(empty.text).toBe("Só título\n\nDescrição pública");
    const truncated = snapshotFromHtml(
      `<article>${"<p>palavra </p>".repeat(12000)}</article>`,
      { kind: "text" },
      now,
    );
  ...
  it("manual text is user_text and never claims full-page access", () => {
    const manual = snapshotFromUserText(
      "conteúdo colado ".repeat(30),
      "Título",
      { kind: "item", contentId: "medium:1" },
      now,
    );
  ...
  it("decodes entities and keeps headings", () => {
    expect(
      extractArticle(
        "<article><h2>Parte &amp; todo</h2><p>a&#233;b</p></article>",
      ).text,
    ).toBe("## Parte & todo\naéb");
  });
});
