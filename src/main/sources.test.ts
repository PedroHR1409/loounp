...
describe("source connectors", () => {
  it("maps legacy topics to Dev.to tags and deduplicates normalized articles", async () => {
    ...
    expect(
      new URL(String(fetchMock.mock.calls[0][0])).searchParams.get("tag"),
    ).toBe("ai");
  ...
  it("derives normalized Dev.to tags only from a confirmed profile and tolerates a failed tag", async () => {
    ...
    const queriedTags = fetchMock.mock.calls.map(([input]) =>
  ...
  it("does not use draft profile terms", async () => {
    ...
    await expect(
      ...
      } as unknown as ConfirmedInterestProfile),
    ).resolves.toEqual([]);
  ...
  it("keeps successful Medium feeds when another configured feed fails", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      return new Response(
        "<rss><channel><item><title>Successful</title><link>https://medium.com/@a/story</link></item></channel></rss>",
        { status: 200 },
      );
    });
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("rejects a non-Medium RSS URL before requesting it", async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    await expect(fetchMedium(["https://example.com/rss.xml"])).rejects.toThrow(
      "Feed Medium inválido",
    );
    expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects external RSS redirects and oversized feed bodies", async () => {
    ...
    await expect(
      fetchMedium(["https://medium.com/feed/tag/ai"]),
